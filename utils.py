...
class myResizeKeepRatio:
    ...
    def __call__(self, data):
        image, mask = data
        h, w = image.shape[-2:]
        scale = min(self.size_h / h, self.size_w / w)
        ...
        # 제로 패딩 (중앙 정렬)
        pad_h = self.size_h - new_h
        pad_w = self.size_w - new_w
...
class myNormalize:
    def __init__(self, data_name, train=True):
        if data_name == 'isic18':
            ...
            if train:
                self.mean = 157.950   # round(157.94995471, 3)
                self.std  = 49.314    # round(49.31427339, 3)
            else:
                self.mean = 157.950
                self.std  = 49.314
