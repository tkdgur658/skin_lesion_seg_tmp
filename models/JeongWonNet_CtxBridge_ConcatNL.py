"""
JeongWonNet_CtxBridge Variant: Concat + NonLinear

변경점:
    Concat Fusion + NonLinear Projection 조합

    ctx_cat = concat(ctx_self, ctx_enc)
    ctx_fused = GELU(LayerNorm(Linear(ctx_cat)))

특징:
    - Concat: 정보 보존
    - NonLinear: 복잡한 상호작용 학습
    - 두 장점 결합
"""
import torch
...
class PRCM_Bridge_ConcatNL(nn.Module):
    """
    PRCM with Concat Fusion + NonLinear Projection

    구조:
        ctx_cat = concat(ctx_self, ctx_enc)
        ctx_fused = Dropout(GELU(LayerNorm(Linear(ctx_cat))))
    """
    def __init__(self, channels, enc_channels, num_basis=8, dropout_rate=0.5, bridge_dropout=0.2):
        ...
        # Concat + NonLinear fusion with dropout
        self.ctx_fusion = nn.Sequential(
            nn.Linear(channels + enc_channels, channels, bias=False),
...
class DWBlock_Bridge_ConcatNL(nn.Module):
    def __init__(self, in_channels, out_channels, enc_channels, kernel_size=7, num_basis=8, dropout_rate=0.5, bridge_dropout=0.2):
        ...
        self.prcm = PRCM_Bridge_ConcatNL(out_channels, enc_channels, num_basis=num_basis, dropout_rate=dropout_rate, bridge_dropout=bridge_dropout)
...
class JeongWonNet_CtxBridge_ConcatNL(nn.Module):
    """
    CtxBridge with Concat + NonLinear
    ...
    def __init__(self,
        ...
        # Decoder with ConcatNL Bridge (bridge_dropout 적용)
        self.decoder1 = DWBlock_Bridge_ConcatNL(c_list[5], c_list[4], c_list[4], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder2 = DWBlock_Bridge_ConcatNL(c_list[4], c_list[3], c_list[3], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder3 = DWBlock_Bridge_ConcatNL(c_list[3], c_list[2], c_list[2], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder4 = DWBlock_Bridge_ConcatNL(c_list[2], c_list[1], c_list[1], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder5 = DWBlock_Bridge_ConcatNL(c_list[1], c_list[0], c_list[0], kernel_size, num_basis, dropout_rate, bridge_dropout)
    ...
    def forward(self, x):
        ...
        # Decoder with concat+nonlinear bridge
    ...
    def switch_to_deploy(self):
        for m in self.modules():
            if isinstance(m, (DWBlock, DWBlock_Bridge_ConcatNL)):
                m.switch_to_deploy()


if __name__ == "__main__":
    print("=" * 60)
    print("JeongWonNet_CtxBridge_ConcatNL Test")
    print("Concat Fusion + NonLinear Projection")
    print("=" * 60)

    model = JeongWonNet_CtxBridge_ConcatNL(
        num_classes=1,
        input_channels=3,
        c_list=[24, 48, 64, 96, 128, 192],
        kernel_size=7,
    ...
    print(f"ConcatNL Fusion Parameters: {fusion_params:,}")

    print("\n[Concat + NonLinear 구조]")
    print("  ctx_cat = concat(ctx_self, ctx_enc)")
    print("  ctx_fused = GELU(LayerNorm(Linear(ctx_cat)))")
    print("  정보 보존 + 복잡한 상호작용 학습")
