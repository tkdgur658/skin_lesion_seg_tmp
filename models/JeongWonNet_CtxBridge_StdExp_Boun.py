...
StdExp 대비 PRCM 강화:
    - bottleneck: 8 → 16
    - 비선형성: sigmoid만 → ReLU → ReLU → sigmoid (3-layer MLP)

PRCM flow:
    GAP → [B,C] → basis.t() → [B,16] → ReLU → Linear → [B,16] → ReLU → Linear → [B,C] → sigmoid
...
class PRCM(nn.Module):
    ...
    def forward(self, x, return_ctx=False):
        ...
        w = self.mlp(coeff).sigmoid().unsqueeze(-1).unsqueeze(-1)
...
class JeongWonNet_CtxBridge_StdExp_Boun(nn.Module):
    """
    Context Bridge with Std Conv (shallow) + Expansion (deep)
    + Boosted PRCM (3-layer MLP, bottleneck=16)
...
if __name__ == "__main__":
    model = JeongWonNet_CtxBridge_StdExp_Boun()
