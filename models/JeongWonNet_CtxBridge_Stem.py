"""
JeongWonNet_CtxBridge with Proper Stem

기존 문제:
    첫 번째 DWBlock에서 3ch → 24ch 변환 시 1x1 conv 사용
    → 공간 정보 추출 없이 채널만 확장

해결:
    Stem block으로 3x3 conv 사용하여 초기 특징 추출
    → RGB 이미지에서 저수준 특징(엣지, 텍스처) 효과적 추출
"""
import torch
...
class Stem(nn.Module):
    """
    Initial feature extraction from RGB
    3x3 conv로 공간 정보 추출하며 채널 확장
    """
...
class JeongWonNet_CtxBridge_Stem(nn.Module):
    """
    Context Bridge with Proper Stem

    Stem: 3x3 conv로 RGB → c_list[0] 변환 (공간 정보 보존)
    encoder1: c_list[0] → c_list[0] (1x1 없이 바로 DW conv)
    """
    def __init__(self,
                 num_classes=1,
                 input_channels=3,
        ...
        # Stem: 3x3 conv for initial feature extraction
        self.stem = Stem(input_channels, c_list[0])

        # Encoder (encoder1은 이제 in_ch == out_ch이므로 1x1 없음)
        self.encoder1 = DWBlock(c_list[0], c_list[0], kernel_size, num_basis, dropout_rate)
...
if __name__ == "__main__":
    model = JeongWonNet_CtxBridge_Stem()
    ...
    print(f"Stem params: {stem_params:,}")
