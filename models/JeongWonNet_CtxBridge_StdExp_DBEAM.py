"""
JeongWonNet_CtxBridge_StdExp with DBEAM at Bottleneck

원본 JeongWonNet_CtxBridge_StdExp 구조 유지 + AMNet의 DBEAM을 bottleneck에 추가
...
Bottleneck (encoder6 출력 → decoder1 입력 사이):
    DBEAM (Detail Boundary Enhancement Attention Module)
    - Local attention: 1x1 bottleneck (channels → channels//r → channels)
    - Global attention: GAP + 1x1 bottleneck
...
class DBEAM(nn.Module):
    """Detail Boundary Enhancement Attention Module (from AMNet)"""
...
class JeongWonNet_CtxBridge_StdExp_DBEAM(nn.Module):
    """
    Context Bridge with Std Conv (shallow) + Expansion (deep) + DBEAM at Bottleneck

    Shallow (enc1-4, dec3-5): std(3x3)
    Deep (enc5-6, dec1-2): expand(1x1, x4) → project(1x1)
    Bottleneck: encoder6 → DBEAM → decoder1
    """
    def __init__(self,
                 num_classes=1,
                 input_channels=3,
        ...
        # DBEAM at bottleneck
        self.dbeam = DBEAM(c_list[5], r=dbeam_r)
...
if __name__ == "__main__":
    model = JeongWonNet_CtxBridge_StdExp_DBEAM()
    ...
    from JeongWonNet_CtxBridge_StdExp import JeongWonNet_CtxBridge_StdExp
    orig = JeongWonNet_CtxBridge_StdExp()
    orig_params = sum(p.numel() for p in orig.parameters())
    dbeam_params = sum(p.numel() for p in model.parameters())
    print(f"Original params: {orig_params:,}")
    print(f"DBEAM params: {dbeam_params:,}")
    print(f"Added params: {dbeam_params - orig_params:,}")
