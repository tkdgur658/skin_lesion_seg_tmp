"""
JeongWonNet_CtxBridge Variant: Concat Fusion

변경점:
    기존: ctx_fused = ctx_self + proj(ctx_enc)  (덧셈)
    변경: ctx_fused = Linear(concat(ctx_self, ctx_enc))  (Concat 후 Linear)

특징:
    - 정보 보존에 유리 (덧셈은 정보 손실 가능)
    - 파라미터 약간 증가
    - 더 풍부한 상호작용 학습 가능
"""
import torch
...
class PRCM_Bridge_Concat(nn.Module):
    """
    PRCM with Concat Fusion

    기존 (Add):  ctx_fused = ctx_self + proj(ctx_enc)
    변경 (Concat): ctx_fused = dropout(Linear(concat(ctx_self, ctx_enc)))
    ...
    def __init__(self, channels, enc_channels, num_basis=8, dropout_rate=0.5, bridge_dropout=0.2):
        ...
        # Concat fusion: (channels + enc_channels) → channels
        self.ctx_fusion = nn.Linear(channels + enc_channels, channels, bias=False)

        # Bridge dropout - fusion 결과에 적용
    ...
    def forward(self, x, ctx_enc=None):
        ...
        if ctx_enc is not None:
            # Concat fusion with dropout
            ctx_cat = torch.cat([ctx_self, ctx_enc], dim=1)
            ctx_fused = self.ctx_fusion(ctx_cat)
            ctx_fused = self.bridge_dropout(ctx_fused)
        else:
            ctx_fused = ctx_self
...
class DWBlock_Bridge_Concat(nn.Module):
    def __init__(self, in_channels, out_channels, enc_channels, kernel_size=7, num_basis=8, dropout_rate=0.5, bridge_dropout=0.2):
        ...
        self.prcm = PRCM_Bridge_Concat(out_channels, enc_channels, num_basis=num_basis, dropout_rate=dropout_rate, bridge_dropout=bridge_dropout)
...
class JeongWonNet_CtxBridge_Concat(nn.Module):
    """
    CtxBridge with Concat Fusion

    ctx_fused = Linear(concat(ctx_self, ctx_enc))
    """
    def __init__(self,
                 num_classes=1,
                 input_channels=3,
        ...
        # Decoder with Concat Bridge (bridge_dropout 적용)
        self.decoder1 = DWBlock_Bridge_Concat(c_list[5], c_list[4], c_list[4], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder2 = DWBlock_Bridge_Concat(c_list[4], c_list[3], c_list[3], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder3 = DWBlock_Bridge_Concat(c_list[3], c_list[2], c_list[2], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder4 = DWBlock_Bridge_Concat(c_list[2], c_list[1], c_list[1], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder5 = DWBlock_Bridge_Concat(c_list[1], c_list[0], c_list[0], kernel_size, num_basis, dropout_rate, bridge_dropout)
    ...
    def switch_to_deploy(self):
        for m in self.modules():
            if isinstance(m, (DWBlock, DWBlock_Bridge_Concat)):
                m.switch_to_deploy()


if __name__ == "__main__":
    print("=" * 60)
    print("JeongWonNet_CtxBridge_Concat Test")
    print("Fusion: concat(ctx_self, ctx_enc) → Linear")
    print("=" * 60)

    model = JeongWonNet_CtxBridge_Concat(
        num_classes=1,
        input_channels=3,
        c_list=[24, 48, 64, 96, 128, 192],
        kernel_size=7,
    ...
    fusion_params = sum(
        sum(p.numel() for p in m.prcm.ctx_fusion.parameters())
        for m in [model.decoder1, model.decoder2, model.decoder3, model.decoder4, model.decoder5]
    )
    print(f"Concat Fusion Parameters: {fusion_params:,}")

    print("\n[Concat Fusion 구조]")
    print("  ctx_cat = concat(ctx_self, ctx_enc)")
    print("  ctx_fused = Linear(ctx_cat)")
    print("  정보 보존 우수, 파라미터 증가")
