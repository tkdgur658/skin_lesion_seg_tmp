"""
JeongWonNet_CtxBridge with Standard Conv

DWBlock에 standard conv 추가:
    pw_conv (1x1) → dw_conv (7x7 DW) → std_conv (3x3) → prcm

추가 이유:
    - DW conv는 채널 믹싱이 없음
    - Standard conv로 채널 간 정보 교환 추가
    - 더 풍부한 특징 표현
"""
import torch
...
class DWBlock(nn.Module):
    """DWBlock with Standard Conv for channel mixing"""
    def __init__(self, in_channels, out_channels, kernel_size=7, num_basis=8, dropout_rate=0.5, std_kernel=3):
        super().__init__()
        ...
        # Depthwise conv (spatial, no channel mixing)
        ...
        # Standard conv (channel mixing)
        self.std_conv = nn.Sequential(
            nn.Conv2d(out_channels, out_channels, kernel_size=std_kernel, padding=std_kernel // 2, bias=False),
    ...
    def forward(self, x, return_ctx=False):
        ...
        x = self.dw_conv(x)
        x = self.std_conv(x)  # Channel mixing
...
class DWBlock_Bridge(nn.Module):
    """Decoder DWBlock with Standard Conv and Context Bridge"""
    def __init__(self, in_channels, out_channels, enc_channels, kernel_size=7, num_basis=8, dropout_rate=0.5, bridge_dropout=0.2, std_kernel=3):
        super().__init__()
...
class JeongWonNet_CtxBridge_Std(nn.Module):
    """
    Context Bridge with Standard Conv in DWBlock
    ...
    def __init__(self,
        ...
        # Encoder
        self.encoder1 = DWBlock(input_channels, c_list[0], kernel_size, num_basis, dropout_rate, std_kernel)
        self.encoder2 = DWBlock(c_list[0], c_list[1], kernel_size, num_basis, dropout_rate, std_kernel)
        self.encoder3 = DWBlock(c_list[1], c_list[2], kernel_size, num_basis, dropout_rate, std_kernel)
        self.encoder4 = DWBlock(c_list[2], c_list[3], kernel_size, num_basis, dropout_rate, std_kernel)
        self.encoder5 = DWBlock(c_list[3], c_list[4], kernel_size, num_basis, dropout_rate, std_kernel)
        self.encoder6 = DWBlock(c_list[4], c_list[5], kernel_size, num_basis, dropout_rate, std_kernel)
        ...
        # Decoder with Bridge
        self.decoder1 = DWBlock_Bridge(c_list[5], c_list[4], c_list[4], kernel_size, num_basis, dropout_rate, bridge_dropout, std_kernel)
        self.decoder2 = DWBlock_Bridge(c_list[4], c_list[3], c_list[3], kernel_size, num_basis, dropout_rate, bridge_dropout, std_kernel)
        self.decoder3 = DWBlock_Bridge(c_list[3], c_list[2], c_list[2], kernel_size, num_basis, dropout_rate, bridge_dropout, std_kernel)
        self.decoder4 = DWBlock_Bridge(c_list[2], c_list[1], c_list[1], kernel_size, num_basis, dropout_rate, bridge_dropout, std_kernel)
        self.decoder5 = DWBlock_Bridge(c_list[1], c_list[0], c_list[0], kernel_size, num_basis, dropout_rate, bridge_dropout, std_kernel)
...
if __name__ == "__main__":
    model = JeongWonNet_CtxBridge_Std()
