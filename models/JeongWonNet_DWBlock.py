...
class JeongWonNet_DWBlock(nn.Module):
    """
    JeongWonNet with DWBlock
...
if __name__ == "__main__":
    print("=" * 60)
    print("DWBlock Parameter Comparison (1x1 위치에 따른 차이)")
    print("=" * 60)

    # 파라미터 비교: 64 -> 128 채널 확장 시
    in_ch, out_ch = 64, 128
    kernel_size = 7

    # 현재 방식: RepConv(in) -> 1x1 -> PRCM
    # RepConv params: in_ch * 7 * 7 + in_ch * 1 + BNs
    repconv_in = in_ch * 49 + in_ch + in_ch * 2 * 3  # kxk + 1x1 + 3 BNs
    pw_conv = in_ch * out_ch
    prcm = 8 * out_ch * 2  # basis + fuser

    print(f"\n[현재] RepConv({in_ch}ch) -> 1x1 -> PRCM({out_ch}ch)")
    print(f"  RepConv: {repconv_in:,}")
    print(f"  1x1 Conv: {pw_conv:,}")
    print(f"  PRCM: {prcm:,}")
    print(f"  Total: {repconv_in + pw_conv + prcm:,}")

    # 이전 방식: 1x1 -> RepConv(out) -> PRCM
    repconv_out = out_ch * 49 + out_ch + out_ch * 2 * 3
    print(f"\n[이전] 1x1 -> RepConv({out_ch}ch) -> PRCM({out_ch}ch)")
    print(f"  1x1 Conv: {pw_conv:,}")
    print(f"  RepConv: {repconv_out:,}")
    print(f"  PRCM: {prcm:,}")
    print(f"  Total: {pw_conv + repconv_out + prcm:,}")

    saved = (pw_conv + repconv_out + prcm) - (repconv_in + pw_conv + prcm)
    print(f"\n절약: {saved:,} params ({saved / (pw_conv + repconv_out + prcm) * 100:.1f}%)")

    print("\n" + "=" * 60)
    print("JeongWonNet_DWBlock Test")
    print("=" * 60)

    model = JeongWonNet_DWBlock(
        num_classes=1,
        input_channels=3,
        c_list=[24, 48, 64, 96, 128, 192],
        kernel_size=7,
    ...
    with torch.no_grad():
        out_deploy = model(x)
    print(f"Deploy Mode Output: {out_deploy.shape}")
