...
class PRCM(nn.Module):
    """FlashAttention의 block-wise 처리"""
    def __init__(self, channels, num_basis=8, block_size=64):
        super().__init__()
        self.block_size = block_size
        self.num_basis = num_basis
        self.basis = nn.Parameter(torch.randn(num_basis, channels))
        
        # Channel을 block 단위로 처리
        self.num_blocks = (channels + block_size - 1) // block_size
        self.fusers = nn.ModuleList([
            nn.Linear(num_basis, min(block_size, channels - i*block_size), 
                     bias=False)
    ...
    def forward(self, x):
        ...
        # Block-wise processing (메모리 효율적)
        weights = []
        for i, fuser in enumerate(self.fusers):
            start = i * self.block_size
            end = min(start + self.block_size, C)
            
            # Block별 basis projection
            block_ctx = ctx[:, start:end]
            coeff = block_ctx @ self.basis[:, start:end].t()
...
class JeongWonNet77_Rep256Basis8S24_EffDown(nn.Module):
