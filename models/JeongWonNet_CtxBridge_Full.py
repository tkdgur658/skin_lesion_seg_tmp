"""
JeongWonNet_CtxBridge_Full

Context 전달 방식:
    1. Encoder → 다음 Encoder (sequential)
    2. Encoder → 대응 Decoder (bridge)

구조:
    enc1 → ctx1 ──┬──→ enc2 → ctx2 ──┬──→ enc3 → ...
                  │                   │
                  └─────────────────────────────→ dec5
                                      └─────────→ dec4

장점:
    - Encoder 내에서 이전 스테이지 정보 활용
    - Decoder에서 대응 encoder 정보 활용
    - 더 풍부한 정보 흐름
"""
import torch
...
class PRCM(nn.Module):
    """Basic PRCM - returns ctx for bridge"""
...
class PRCM_WithPrevCtx(nn.Module):
    """
    PRCM with Previous Encoder Context (for Encoder stages 2~6)

    이전 encoder의 ctx를 받아서 현재 PRCM에 활용
    ctx_fused = ctx_self + proj(ctx_prev)
    """
    def __init__(self, channels, prev_channels, num_basis=8, dropout_rate=0.5, bridge_dropout=0.2):
        super().__init__()
        self.num_basis = num_basis
        self.channels = channels
    ...
    def forward(self, x, ctx_prev=None, return_ctx=False):
        B, C, H, W = x.shape
        ...
        if ctx_prev is not None:
            ...
            ctx_prev_proj = self.bridge_dropout(ctx_prev_proj)
        ...
        if return_ctx:
            return x * w, ctx_self  # 자신의 ctx 반환 (fused 아닌 원본)
        return x * w


class PRCM_Bridge(nn.Module):
    """PRCM with Encoder Context Bridge (for Decoder)"""
...
class DWBlock(nn.Module):
    """First Encoder DWBlock (no previous ctx)"""
...
class DWBlock_WithPrevCtx(nn.Module):
    """Encoder DWBlock with Previous Context (enc2~enc6)"""
    def __init__(self, in_channels, out_channels, prev_channels, kernel_size=7,
        ...
        self.prcm = PRCM_WithPrevCtx(
            out_channels, prev_channels, num_basis=num_basis,
    ...
    def forward(self, x, ctx_prev=None, return_ctx=False):
        ...
        if return_ctx:
            return self.prcm(x, ctx_prev, return_ctx=True)
...
class DWBlock_Bridge(nn.Module):
    """Decoder DWBlock with Encoder Context Bridge"""
...
class JeongWonNet_CtxBridge_Full(nn.Module):
    """
    Full Context Bridge

    Context Flow:
        Encoder: ctx1 → enc2, ctx2 → enc3, ..., ctx5 → enc6
        Decoder: ctx1 → dec5, ctx2 → dec4, ..., ctx5 → dec1
    ...
    def __init__(self,
        ...
        # Encoder 1 (no previous ctx)
        self.encoder1 = DWBlock(input_channels, c_list[0], kernel_size, num_basis, dropout_rate)

        # Encoder 2~6 (with previous ctx)
        self.encoder2 = DWBlock_WithPrevCtx(c_list[0], c_list[1], c_list[0], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.encoder3 = DWBlock_WithPrevCtx(c_list[1], c_list[2], c_list[1], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.encoder4 = DWBlock_WithPrevCtx(c_list[2], c_list[3], c_list[2], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.encoder5 = DWBlock_WithPrevCtx(c_list[3], c_list[4], c_list[3], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.encoder6 = DWBlock_WithPrevCtx(c_list[4], c_list[5], c_list[4], kernel_size, num_basis, dropout_rate, bridge_dropout)
        ...
        # Decoder with Encoder Context Bridge
    ...
    def forward(self, x):
        ...
        # Encoder with sequential context flow
        e1_out, ctx1 = self.encoder1(x, return_ctx=True)
        e1 = F.max_pool2d(e1_out, 2)

        e2_out, ctx2 = self.encoder2(e1, ctx_prev=ctx1, return_ctx=True)
        e2 = F.max_pool2d(e2_out, 2)

        e3_out, ctx3 = self.encoder3(e2, ctx_prev=ctx2, return_ctx=True)
        e3 = F.max_pool2d(e3_out, 2)

        e4_out, ctx4 = self.encoder4(e3, ctx_prev=ctx3, return_ctx=True)
        e4 = F.max_pool2d(e4_out, 2)

        e5_out, ctx5 = self.encoder5(e4, ctx_prev=ctx4, return_ctx=True)
        e5 = F.max_pool2d(e5_out, 2)

        e6 = self.encoder6(e5, ctx_prev=ctx5)
    ...
    def switch_to_deploy(self):
        for m in self.modules():
            if isinstance(m, (DWBlock, DWBlock_WithPrevCtx, DWBlock_Bridge)):
                m.switch_to_deploy()


if __name__ == "__main__":
    print("=" * 60)
    print("JeongWonNet_CtxBridge_Full Test")
    print("Encoder→Encoder + Encoder→Decoder Context Bridge")
    print("=" * 60)

    model = JeongWonNet_CtxBridge_Full(
        num_classes=1,
        input_channels=3,
        c_list=[24, 48, 64, 96, 128, 192],
        kernel_size=7,
    ...
    # Encoder bridge params (enc2~enc6)
    enc_bridge_params = sum(
        sum(p.numel() for p in m.prcm.ctx_proj.parameters())
        for m in [model.encoder2, model.encoder3, model.encoder4, model.encoder5, model.encoder6]
    )
    print(f"Encoder Bridge Parameters: {enc_bridge_params:,}")

    # Decoder bridge params
    dec_bridge_params = sum(
        sum(p.numel() for p in m.prcm.ctx_proj.parameters())
        for m in [model.decoder1, model.decoder2, model.decoder3, model.decoder4, model.decoder5]
        if hasattr(m.prcm.ctx_proj, 'parameters')
    )
    print(f"Decoder Bridge Parameters: {dec_bridge_params:,}")

    print("\n[Full Context Bridge 구조]")
    print("  Encoder: ctx1→enc2, ctx2→enc3, ctx3→enc4, ctx4→enc5, ctx5→enc6")
    print("  Decoder: ctx5→dec1, ctx4→dec2, ctx3→dec3, ctx2→dec4, ctx1→dec5")
    print("  양방향 context 전달로 더 풍부한 정보 흐름")
