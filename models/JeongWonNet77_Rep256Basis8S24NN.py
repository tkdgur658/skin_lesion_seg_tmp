...
class JeongWonNet77_Rep256Basis8S24NN(nn.Module):
    """
    ...
    def forward(self, x):
        ...
        d4 = F.interpolate(self.decoder2(d5), scale_factor=2, mode='nearest')  # [B, c3, H/16, W/16]
        d4 = d4 + e4                                # Simple skip connection

        # d3: upsampling 후 e3와 더함
        d3 = F.interpolate(self.decoder3(d4), scale_factor=2, mode='nearest')  # [B, c2, H/8, W/8]
        d3 = d3 + e3                                # Simple skip connection

        # d2: upsampling 후 e2와 더함
        d2 = F.interpolate(self.decoder4(d3), scale_factor=2, mode='nearest')  # [B, c1, H/4, W/4]
        d2 = d2 + e2                                # Simple skip connection

        # d1: upsampling 후 e1과 더함
        d1 = F.interpolate(self.decoder5(d2), scale_factor=2, mode='nearest')  # [B, c0, H/2, W/2]
        d1 = d1 + e1                                # Simple skip connection

        # Final segmentation map
        out = F.interpolate(self.final(d1), scale_factor=2, mode='nearest')    # [B, num_classes, H, W]
