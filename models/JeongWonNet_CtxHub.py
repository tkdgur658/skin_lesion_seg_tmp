"""
JeongWonNet with Global Context Hub

Context Hub 방식:
    모든 Encoder context를 Hub에 모아서 Decoder에서 참조

    Encoder: ctx1, ctx2, ..., ctx5 → Hub (concat & compress)
    Decoder: 각 stage에서 Hub 참조

특징:
    - 전역 멀티스케일 정보 활용
    - 모든 스케일의 context를 한번에 참조
    - 더 풍부한 정보지만 파라미터 증가
"""
import torch
...
class ContextHub(nn.Module):
    """
    Global Context Hub

    Concat all encoder contexts and compress to hub_dim
    """
    def __init__(self, ctx_channels_list, hub_dim=64):
        super().__init__()
        total_channels = sum(ctx_channels_list)
        self.hub_dim = hub_dim

        # Compress concatenated contexts
        self.compress = nn.Sequential(
            nn.Linear(total_channels, hub_dim * 2, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(hub_dim * 2, hub_dim, bias=False)
        )

    def forward(self, ctx_list):
        # ctx_list: [ctx1, ctx2, ..., ctx5]
        ctx_cat = torch.cat(ctx_list, dim=1)  # (B, total_channels)
        hub = self.compress(ctx_cat)  # (B, hub_dim)
        return hub


class PRCM_Hub(nn.Module):
    """
    PRCM with Global Context Hub

    ctx_fused = ctx_self + proj(hub)
    """
    def __init__(self, channels, hub_dim=64, num_basis=8, dropout_rate=0.5):
        super().__init__()
        self.num_basis = num_basis
        self.channels = channels
        ...
        # Hub projection
        self.hub_proj = nn.Linear(hub_dim, channels, bias=False)

    def forward(self, x, hub=None):
        B, C, H, W = x.shape

        ctx_self = x.mean(dim=[2, 3])

        if hub is not None:
            hub_proj = self.hub_proj(hub)
            ctx_fused = ctx_self + hub_proj
        else:
            ctx_fused = ctx_self
...
class DWBlock_Hub(nn.Module):
    """Decoder DWBlock with Context Hub"""
    def __init__(self, in_channels, out_channels, hub_dim=64,
        ...
        self.prcm = PRCM_Hub(out_channels, hub_dim, num_basis=num_basis, dropout_rate=dropout_rate)
...
class JeongWonNet_CtxHub(nn.Module):
    """
    Global Context Hub

    Encoder: ctx1, ctx2, ctx3, ctx4, ctx5 → Hub
    Decoder: 각 stage에서 Hub 참조
    """
    def __init__(self,
                 num_classes=1,
        ...
        # Context Hub
        ctx_channels = [c_list[0], c_list[1], c_list[2], c_list[3], c_list[4]]
        self.context_hub = ContextHub(ctx_channels, hub_dim)
        ...
        # Decoder with Hub
        self.decoder1 = DWBlock_Hub(c_list[5], c_list[4], hub_dim, kernel_size, num_basis, dropout_rate)
        self.decoder2 = DWBlock_Hub(c_list[4], c_list[3], hub_dim, kernel_size, num_basis, dropout_rate)
        self.decoder3 = DWBlock_Hub(c_list[3], c_list[2], hub_dim, kernel_size, num_basis, dropout_rate)
        self.decoder4 = DWBlock_Hub(c_list[2], c_list[1], hub_dim, kernel_size, num_basis, dropout_rate)
        self.decoder5 = DWBlock_Hub(c_list[1], c_list[0], hub_dim, kernel_size, num_basis, dropout_rate)
    ...
    def forward(self, x):
        ...
        # Build context hub
        hub = self.context_hub([ctx1, ctx2, ctx3, ctx4, ctx5])

        # Decoder with hub
        d5 = self.decoder1(e6, hub=hub) + e5
        d4 = F.interpolate(self.decoder2(d5, hub=hub), scale_factor=2, mode='bilinear', align_corners=True) + e4
        d3 = F.interpolate(self.decoder3(d4, hub=hub), scale_factor=2, mode='bilinear', align_corners=True) + e3
        d2 = F.interpolate(self.decoder4(d3, hub=hub), scale_factor=2, mode='bilinear', align_corners=True) + e2
        d1 = F.interpolate(self.decoder5(d2, hub=hub), scale_factor=2, mode='bilinear', align_corners=True) + e1
    ...
    def switch_to_deploy(self):
        for m in self.modules():
            if isinstance(m, (DWBlock, DWBlock_Hub)):
                m.switch_to_deploy()


if __name__ == "__main__":
    print("=" * 60)
    print("JeongWonNet_CtxHub Test")
    print("Global Context Hub")
    print("=" * 60)

    model = JeongWonNet_CtxHub(
        num_classes=1,
        input_channels=3,
        c_list=[24, 48, 64, 96, 128, 192],
        kernel_size=7,
    ...
    hub_params = sum(p.numel() for p in model.context_hub.parameters())
    print(f"Context Hub Parameters: {hub_params:,}")

    hub_proj_params = sum(
        sum(p.numel() for p in m.prcm.hub_proj.parameters())
        for m in [model.decoder1, model.decoder2, model.decoder3, model.decoder4, model.decoder5]
    )
    print(f"Hub Projection Parameters: {hub_proj_params:,}")

    print("\n[Global Context Hub 구조]")
    print("  Encoder: ctx1(24), ctx2(48), ctx3(64), ctx4(96), ctx5(128)")
    print("  Hub: concat(360) → compress → hub(64)")
    print("  Decoder: ctx_fused = ctx_self + proj(hub)")
    print("  모든 스케일 정보를 전역적으로 참조")
