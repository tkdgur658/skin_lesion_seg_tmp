...
class MHorUNet(nn.Module):
    ...
    def __init__(self, num_classes=1, input_channels=3,layer_scale_init_value=1e-6,gnconv=gnconv, block=Block,pretrained=None,
        ...
        self.encoder3 = nn.Sequential(
            *[block(dim=c_list[1], drop_path=dp_rates[0 + j],
            layer_scale_init_value=layer_scale_init_value, gnconv=gnconv[0]) for j in range(depths[0])],
            nn.Conv2d(c_list[1], c_list[2], 3, stride=1, padding=1),
        )

        self.encoder4 = nn.Sequential(
            *[block(dim=c_list[2], drop_path=dp_rates[2 + j],
            layer_scale_init_value=layer_scale_init_value, gnconv=gnconv[1]) for j in range(depths[1])],
        ...
        self.encoder5 = nn.Sequential(
            *[block(dim=c_list[3], drop_path=dp_rates[5 + j],
            layer_scale_init_value=layer_scale_init_value, gnconv=gnconv[2]) for j in range(depths[2])],
            nn.Conv2d(c_list[3], c_list[4], 3, stride=1, padding=1),
            )

        self.encoder6 = nn.Sequential(
            *[block(dim=c_list[4], drop_path=dp_rates[23 + j],
            layer_scale_init_value=layer_scale_init_value, gnconv=gnconv[3]) for j in range(depths[3])],
        ...
        self.decoder1 = nn.Sequential(
            ...
            *[block(dim=c_list[4], drop_path=dp_rates[24],
            layer_scale_init_value=layer_scale_init_value, gnconv=gnconv[3]) for j in range(depths[3])],
            )
