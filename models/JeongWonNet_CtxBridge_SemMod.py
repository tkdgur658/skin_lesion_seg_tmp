"""
JeongWonNet with Context Bridge + Semantic Modulation Fusion

Skip Connection 방식:
    High-level 특징이 Low-level 특징을 조절 (Modulation)
    gamma, beta = modulator(dec_feat)
    enc_modulated = enc_feat * (1 + gamma) + beta

특징:
    - High-level semantic 정보로 low-level 특징 공간적 조절
    - FiLM (Feature-wise Linear Modulation) 스타일
    - 엣지 강화/노이즈 억제
"""
import torch
...
class SemanticModulationFusion(nn.Module):
    """
    Semantic Modulation Fusion (FiLM-style)
    High-level 특징이 Low-level 특징을 공간적으로 조절
    """
...
class JeongWonNet_CtxBridge_SemMod(nn.Module):
    """
    Context Bridge + Semantic Modulation Fusion

    High-level이 Low-level을 FiLM 스타일로 조절
    """
    def __init__(self,
                 num_classes=1,
                 input_channels=3,
        ...
        # Semantic Modulation Fusion modules
        self.fusion5 = SemanticModulationFusion(c_list[4], c_list[4])
        self.fusion4 = SemanticModulationFusion(c_list[3], c_list[3])
        self.fusion3 = SemanticModulationFusion(c_list[2], c_list[2])
        self.fusion2 = SemanticModulationFusion(c_list[1], c_list[1])
        self.fusion1 = SemanticModulationFusion(c_list[0], c_list[0])
    ...
    def forward(self, x):
        ...
        # Decoder with context bridge + Semantic Modulation fusion
...
if __name__ == "__main__":
    model = JeongWonNet_CtxBridge_SemMod()
