"""
JeongWonNet_CtxBridge with 1x1 Conv (shallow) + Expansion (deep)

얕은 레이어 (encoder1-4, decoder3-5):
    pw(1x1) → dw(7x7) → pw(1x1) → prcm

깊은 레이어 (encoder5-6, decoder1-2):
    pw(1x1) → dw(7x7) → expand(1x1, x4) → project(1x1) → prcm

StdExp에서 얕은 레이어의 3x3을 1x1로 변경:
    - 더 가벼운 모델
    - DW conv 이후 간단한 채널 믹싱
"""
import torch
...
class DWBlock_Pw(nn.Module):
    """DWBlock with 1x1 Pointwise Conv (for shallow layers)"""
    def __init__(self, in_channels, out_channels, kernel_size=7, num_basis=8, dropout_rate=0.5):
        ...
        # 1x1 pointwise conv for channel mixing
...
class DWBlock_Pw_Bridge(nn.Module):
    """Decoder DWBlock with 1x1 Pointwise Conv (for shallow layers)"""
...
class JeongWonNet_CtxBridge_PwExp(nn.Module):
    """
    Context Bridge with 1x1 Pw Conv (shallow) + Expansion (deep)

    Shallow (enc1-4, dec3-5): pw(1x1) → dw(7x7) → pw(1x1) → prcm
    Deep (enc5-6, dec1-2): pw(1x1) → dw(7x7) → expand(1x1, x4) → project(1x1) → prcm

    StdExp에서 얕은 레이어의 3x3을 1x1로 변경한 경량 버전
    """
    def __init__(self,
                 num_classes=1,
                 input_channels=3,
        ...
        # Encoder - Shallow layers with 1x1 Pw Conv
        self.encoder1 = DWBlock_Pw(input_channels, c_list[0], kernel_size, num_basis, dropout_rate)
        self.encoder2 = DWBlock_Pw(c_list[0], c_list[1], kernel_size, num_basis, dropout_rate)
        self.encoder3 = DWBlock_Pw(c_list[1], c_list[2], kernel_size, num_basis, dropout_rate)
        self.encoder4 = DWBlock_Pw(c_list[2], c_list[3], kernel_size, num_basis, dropout_rate)
        ...
        # Shallow layers with 1x1 Pw Conv
        self.decoder3 = DWBlock_Pw_Bridge(c_list[3], c_list[2], c_list[2], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder4 = DWBlock_Pw_Bridge(c_list[2], c_list[1], c_list[1], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder5 = DWBlock_Pw_Bridge(c_list[1], c_list[0], c_list[0], kernel_size, num_basis, dropout_rate, bridge_dropout)
    ...
    def switch_to_deploy(self):
        for m in self.modules():
            if isinstance(m, (DWBlock_Pw, DWBlock_Exp, DWBlock_Pw_Bridge, DWBlock_Exp_Bridge)):
                m.switch_to_deploy()


if __name__ == "__main__":
    model = JeongWonNet_CtxBridge_PwExp()
