"""
JeongWonNet_CtxBridge_StdExp — Deterministic Version

재현성 보장 방법:
  - 디코더/최종출력의 bilinear interpolation →
    고정 가중치(frozen) depthwise ConvTranspose2d로 대체
  - ConvTranspose2d backward = Conv2d forward → cudnn.deterministic=True로 결정적
  - forward 출력은 bilinear upsampling과 동일 (학습 가능 파라미터 없음)
  - 딥 수퍼비전(보조 loss)만 nearest 사용 (가변 크기, 학습 품질 영향 미미)
...
class JeongWonNet_CtxBridge_StdExp_Det(nn.Module):
    """
    Context Bridge with Std Conv (shallow) + Expansion (deep) — Deterministic
    ...
    def __init__(self,
        ...
        self.up2 = BilinearUp(c_list[3])   # 96ch
        self.up3 = BilinearUp(c_list[2])   # 64ch
        self.up4 = BilinearUp(c_list[1])   # 48ch
        self.up5 = BilinearUp(c_list[0])   # 24ch
        self.up_final = BilinearUp(num_classes)  # 1ch
...
if __name__ == "__main__":
    model = JeongWonNet_CtxBridge_StdExp_Det()
