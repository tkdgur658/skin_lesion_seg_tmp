"""
JeongWonNet_CtxBridge_StdExp_TConv_DWCF (Depthwise on Concatenated Features, Deterministic)

디코더에서 encoder+decoder feature를 concat한 뒤,
depthwise conv가 전체 채널에 대해 spatial processing → pointwise로 채널 축소

기존 CF:   cat → pw(1x1, reduce) → dw(reduced) → std/exp → prcm
DWCF:      cat → dw(full, 7x7)  → pw(1x1, reduce) → std/exp → prcm
                 ↑ spatial conv가 encoder+decoder 전체 채널을 보고 처리
...
class DWBlock_Std(nn.Module):
    """Encoder DWBlock with 3x3 Standard Conv (for shallow layers)"""
...
class DWBlock_Exp(nn.Module):
    """Encoder DWBlock with 1x1 Expansion-Projection (for deep layers)"""
...
# ===================== Decoder Blocks (DWCF: DW on full concat, then PW reduce) =====================

class DWBlock_Std_Bridge_DWCF(nn.Module):
    """Decoder DWBlock — dw on concat channels → pw reduce → std 3x3 → prcm (shallow)

    cat(dec, enc) → dw(full_ch, 7x7) → pw(1x1, full→out) → std(3x3) → prcm_bridge
    """
    def __init__(self, cat_channels, out_channels, enc_channels, kernel_size=7, num_basis=8, dropout_rate=0.5, bridge_dropout=0.2):
        super().__init__()

        # depthwise on full concatenated channels — spatial processing before reduction
        self.dw_conv = RepConv(
        ...
        # pointwise to reduce channels — cross-channel mixing after spatial processing
        self.pw_conv = nn.Conv2d(cat_channels, out_channels, kernel_size=1, bias=False)
...
class DWBlock_Exp_Bridge_DWCF(nn.Module):
    """Decoder DWBlock — dw on concat channels → pw reduce → exp-proj → prcm (deep)

    cat(dec, enc) → dw(full_ch, 7x7) → pw(1x1, full→out) → expand(1x1) → project(1x1) → prcm_bridge
    """
    def __init__(self, cat_channels, out_channels, enc_channels, kernel_size=7, num_basis=8, dropout_rate=0.5, bridge_dropout=0.2, expansion=4):
        super().__init__()

        # depthwise on full concatenated channels
        self.dw_conv = RepConv(
            cat_channels, cat_channels,
            kernel_size=kernel_size,
            padding=kernel_size // 2,
...
class JeongWonNet_CtxBridge_StdExp_TConv_DWCF(nn.Module):
    """
    Context Bridge with Std Conv (shallow) + Expansion (deep)
    DWCF Decoder: depthwise on full concat channels → pointwise reduce → std/exp → prcm

    디코더 블록 내부에서 encoder+decoder 전체 채널에 대해 spatial processing 보장:
        cat(up(d5), e4) → dw(7x7, 224ch) → pw(1x1, 224→96) → std/exp → prcm
    ...
    def __init__(self,
        ...
        # Decoder — DWCF: dw on full concat → pw reduce → std/exp → prcm
        # Deep layers (Exp)
        self.decoder1 = DWBlock_Exp_Bridge_DWCF(
            c_list[5] + c_list[4], c_list[4], c_list[4],  # 192+128=320 → 128
            kernel_size, num_basis, dropout_rate, bridge_dropout, expansion)
        self.decoder2 = DWBlock_Exp_Bridge_DWCF(
            c_list[4] + c_list[3], c_list[3], c_list[3],  # 128+96=224 → 96
            kernel_size, num_basis, dropout_rate, bridge_dropout, expansion)
        # Shallow layers (Std)
        self.decoder3 = DWBlock_Std_Bridge_DWCF(
            c_list[3] + c_list[2], c_list[2], c_list[2],  # 96+64=160 → 64
            kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder4 = DWBlock_Std_Bridge_DWCF(
            c_list[2] + c_list[1], c_list[1], c_list[1],  # 64+48=112 → 48
            kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder5 = DWBlock_Std_Bridge_DWCF(
            c_list[1] + c_list[0], c_list[0], c_list[0],  # 48+24=72 → 24
            kernel_size, num_basis, dropout_rate, bridge_dropout)
    ...
    def forward(self, x):
        ...
        # Decoder — cat first, dw on full channels, then pw reduce
    ...
    def switch_to_deploy(self):
        for m in self.modules():
            if isinstance(m, (DWBlock_Std, DWBlock_Exp, DWBlock_Std_Bridge_DWCF, DWBlock_Exp_Bridge_DWCF)):
                if hasattr(m, 'switch_to_deploy'):
                    m.switch_to_deploy()


if __name__ == "__main__":
    model = JeongWonNet_CtxBridge_StdExp_TConv_DWCF()
    ...
    dwcf_params = sum(p.numel() for p in model.parameters())
    print(f"Base params: {base_params:,}")
    print(f"DWCF params: {dwcf_params:,}")
    print(f"Overhead:    {dwcf_params - base_params:,} ({dwcf_params / base_params * 100 - 100:+.1f}%)")
