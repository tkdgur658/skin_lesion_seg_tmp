"""
JeongWonNet with Encoder→Decoder Context Bridge + Concat Skip Connection

기존 CtxBridge와 차이점:
    - Skip connection을 덧셈 대신 concat 후 1x1 conv로 채널 축소
...
class SkipFusion(nn.Module):
    """Concat-based Skip Connection Fusion"""
    def __init__(self, dec_channels, enc_channels):
        super().__init__()
        # Concat 후 채널 수 조정
        self.conv = nn.Sequential(
            nn.Conv2d(dec_channels + enc_channels, dec_channels, kernel_size=3, padding=1, bias=False),
    ...
    def forward(self, dec_feat, enc_feat):
        ...
        fused = torch.cat([dec_feat, enc_feat], dim=1)
...
class JeongWonNet_CtxBridge_CatSkip(nn.Module):
    """
    Encoder→Decoder Context Bridge with Concat Skip Connection

    Skip connection: concat + 1x1 conv (기존: add)
    ...
    def __init__(self,
        ...
        # Skip Fusion modules (Concat-based)
        self.skip5 = SkipFusion(c_list[4], c_list[4])  # d5 + e5
        self.skip4 = SkipFusion(c_list[3], c_list[3])  # d4 + e4
        self.skip3 = SkipFusion(c_list[2], c_list[2])  # d3 + e3
        self.skip2 = SkipFusion(c_list[1], c_list[1])  # d2 + e2
        self.skip1 = SkipFusion(c_list[0], c_list[0])  # d1 + e1
    ...
    def forward(self, x):
        ...
        # Decoder with context bridge + concat skip connection
        d5 = self.skip5(self.decoder1(e6, ctx_enc=ctx5), e5)
        d4 = self.skip4(F.interpolate(self.decoder2(d5, ctx_enc=ctx4), scale_factor=2, mode='bilinear', align_corners=True), e4)
        d3 = self.skip3(F.interpolate(self.decoder3(d4, ctx_enc=ctx3), scale_factor=2, mode='bilinear', align_corners=True), e3)
        d2 = self.skip2(F.interpolate(self.decoder4(d3, ctx_enc=ctx2), scale_factor=2, mode='bilinear', align_corners=True), e2)
        d1 = self.skip1(F.interpolate(self.decoder5(d2, ctx_enc=ctx1), scale_factor=2, mode='bilinear', align_corners=True), e1)
...
if __name__ == "__main__":
    print("=" * 60)
    print("JeongWonNet_CtxBridge_CatSkip Test")
    print("Encoder→Decoder Context Bridge + Concat Skip Connection")
    print("=" * 60)

    model = JeongWonNet_CtxBridge_CatSkip(
        num_classes=1,
        input_channels=3,
        c_list=[24, 48, 64, 96, 128, 192],
        kernel_size=7,
    ...
    skip_params = sum(
        ...
        for m in [model.skip1, model.skip2, model.skip3, model.skip4, model.skip5]
    )
    print(f"Skip Fusion Parameters: {skip_params:,}")

    print("\n[Skip Connection 비교]")
    print("  기존 (Add):    d5 = decoder(e6) + e5")
    print("  현재 (Concat): d5 = Conv1x1(cat(decoder(e6), e5))")
