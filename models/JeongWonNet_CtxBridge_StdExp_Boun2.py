...
Boun 대비 PRCM 강화:
    - GAP + GMP concat → 2C 입력으로 채널 통계량 풍부하게
    - MLP에 LayerNorm 추가로 학습 안정화
    - Spatial attention 추가 (16x16 low-res depthwise conv)

PRCM Channel flow:
    GAP+GMP → cat → [B,2C] → basis.t() → [B,16] → LN → ReLU → Linear → LN → ReLU → Linear → [B,C] → sigmoid

PRCM Spatial flow:
    x → AdaptiveAvgPool2d(16) → DWConv×3 → sigmoid → interpolate(H,W)

나머지 아키텍처(RepConv, DWBlock, encoder/decoder, DS 등) 완전 동일
"""
import torch
...
class PRCM(nn.Module):
    def __init__(self, channels, num_basis=16, dropout_rate=0.5, use_spatial=False):
        super().__init__()
        ...
        self.mlp = nn.Sequential(
            nn.LayerNorm(num_basis),
            nn.ReLU(inplace=True),
            nn.Dropout(dropout_rate),
            nn.Linear(num_basis, num_basis, bias=False),
            nn.LayerNorm(num_basis),
            nn.ReLU(inplace=True),
            nn.Linear(num_basis, 2 * channels, bias=False),  # γ + δ
        )
    ...
    def forward(self, x, return_ctx=False):
        ...
        coeff = ctx @ self.basis.t()                  # [B, 16]
        coeff = self.coeff_dropout(coeff)
        film = self.mlp(coeff)                        # [B, 2C]
        gamma = film[:, :C].sigmoid().unsqueeze(-1).unsqueeze(-1)
        delta = film[:, C:].unsqueeze(-1).unsqueeze(-1)
        x = x * gamma + delta

        # Spatial attention (first block only)
        if self.use_spatial:
            s = F.adaptive_avg_pool2d(x, 16)          # [B,C,16,16]
            s = self.spatial(s).sigmoid()              # [B,1,16,16]
            w_spa = F.interpolate(s, (H, W), mode='nearest')
            x = x * w_spa
...
class PRCM_Bridge(nn.Module):
    def __init__(self, channels, enc_channels, num_basis=16, dropout_rate=0.5, bridge_dropout=0.2):
        super().__init__()
        self.num_basis = num_basis
        self.channels = channels
...
class DWBlock_Std(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=7, num_basis=16, dropout_rate=0.5, use_spatial=False):
        super().__init__()
        ...
        self.prcm = PRCM(out_channels, num_basis=num_basis, dropout_rate=dropout_rate, use_spatial=use_spatial)
...
class DWBlock_Exp(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=7, num_basis=16, dropout_rate=0.5, expansion=4):
        super().__init__()
...
class DWBlock_Std_Bridge(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, enc_channels, kernel_size=7, num_basis=16, dropout_rate=0.5, bridge_dropout=0.2):
        super().__init__()
...
class DWBlock_Exp_Bridge(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, enc_channels, kernel_size=7, num_basis=16, dropout_rate=0.5, bridge_dropout=0.2, expansion=4):
        super().__init__()
...
class JeongWonNet_CtxBridge_StdExp_Boun2(nn.Module):
    """
    Context Bridge with Std Conv (shallow) + Expansion (deep)
    + Boosted PRCM v2 (GAP+GMP, LayerNorm MLP, Spatial Attention)

    Shallow (enc1-4, dec3-5): std(3x3)
    Deep (enc5-6, dec1-2): expand(1x1, x4) → project(1x1)
    """
    def __init__(self,
                 num_classes=1,
                 input_channels=3,
        ...
        self.encoder1 = DWBlock_Std(input_channels, c_list[0], kernel_size, num_basis, dropout_rate, use_spatial=True)
        self.encoder2 = DWBlock_Std(c_list[0], c_list[1], kernel_size, num_basis, dropout_rate, use_spatial=True)
        ...
        # Decoder - Deep layers with Expansion
        self.decoder1 = DWBlock_Exp_Bridge(c_list[5], c_list[4], c_list[4], kernel_size, num_basis, dropout_rate, bridge_dropout, expansion)
        self.decoder2 = DWBlock_Exp_Bridge(c_list[4], c_list[3], c_list[3], kernel_size, num_basis, dropout_rate, bridge_dropout, expansion)
...
if __name__ == "__main__":
    model = JeongWonNet_CtxBridge_StdExp_Boun2()
