"""
JeongWonNet_CtxBridge_StdExp without PRCM in Deep Layers — Reproducible (Nearest)

재현성 보장 버전: 모든 bilinear interpolation → nearest로 교체
- CUDA에서 bilinear backward는 atomic operation 사용 → 비결정적
- nearest는 forward/backward 모두 결정적

얕은 레이어 (encoder1-4, decoder3-5):
    pw(1x1) → dw(7x7) → std(3x3) → prcm

깊은 레이어 (encoder5-6, decoder1-2):
    pw(1x1) → dw(7x7) → expand(1x1, x4) → project(1x1)  (PRCM 제거)
"""
import torch
...
class DWBlock_Std(nn.Module):
    """DWBlock with 3x3 Standard Conv + PRCM (for shallow layers)"""
...
class DWBlock_ExpNoPRCM(nn.Module):
    """DWBlock with 1x1 Expansion-Projection, NO PRCM (for deep layers)"""
    def __init__(self, in_channels, out_channels, kernel_size=7, expansion=4):
        super().__init__()
    ...
    def forward(self, x, return_ctx=False):
        ...
        if return_ctx:
            # Still return ctx for bridge (GAP of output)
            ctx = x.mean(dim=[2, 3])
            return x, ctx
        return x
...
class DWBlock_Std_Bridge(nn.Module):
    """Decoder DWBlock with 3x3 Standard Conv + PRCM Bridge (for shallow layers)"""
...
class DWBlock_ExpNoPRCM_Bridge(nn.Module):
    """Decoder DWBlock with 1x1 Expansion-Projection, NO PRCM (for deep layers)"""
    def __init__(self, in_channels, out_channels, kernel_size=7, expansion=4):
        super().__init__()
    ...
    def forward(self, x, ctx_enc=None):
        # ctx_enc is ignored (no PRCM bridge)
        if self.pw_conv is not None:
            x = self.pw_conv(x)
...
class JeongWonNet_CtxBridge_StdExpNoPRCM_Nearest(nn.Module):
    """
    Context Bridge with Std Conv (shallow) + Expansion without PRCM (deep)
    Reproducible version: all bilinear → nearest

    Shallow (enc1-4, dec3-5): std(3x3) + PRCM
    Deep (enc5-6, dec1-2): expand(1x1, x4) → project(1x1), NO PRCM
    """
    def __init__(self,
                 num_classes=1,
                 input_channels=3,
        ...
        # Encoder - Shallow layers with Std Conv + PRCM
        ...
        # Deep layers with Expansion, NO PRCM
        self.encoder5 = DWBlock_ExpNoPRCM(c_list[3], c_list[4], kernel_size, expansion)
        self.encoder6 = DWBlock_ExpNoPRCM(c_list[4], c_list[5], kernel_size, expansion)
        ...
        # Decoder - Deep layers with Expansion, NO PRCM
        self.decoder1 = DWBlock_ExpNoPRCM_Bridge(c_list[5], c_list[4], kernel_size, expansion)
        self.decoder2 = DWBlock_ExpNoPRCM_Bridge(c_list[4], c_list[3], kernel_size, expansion)
        # Shallow layers with Std Conv + PRCM Bridge
    ...
    def forward(self, x):
        ...
        # Deep encoder - still extract ctx for potential use (but no PRCM recalibration)
        e5_out, ctx5 = self.encoder5(e4, return_ctx=True)
        e5 = F.max_pool2d(e5_out, 2)

        e6 = self.encoder6(e5)

        # Decoder — nearest interpolation for reproducibility
        # Deep decoders don't use ctx_enc (no PRCM bridge)
        d5 = self.decoder1(e6) + e5
        d4 = F.interpolate(self.decoder2(d5), scale_factor=2, mode='nearest') + e4
        # Shallow decoders use ctx_enc (PRCM bridge)
        d3 = F.interpolate(self.decoder3(d4, ctx_enc=ctx3), scale_factor=2, mode='nearest') + e3
        d2 = F.interpolate(self.decoder4(d3, ctx_enc=ctx2), scale_factor=2, mode='nearest') + e2
        d1 = F.interpolate(self.decoder5(d2, ctx_enc=ctx1), scale_factor=2, mode='nearest') + e1
    ...
    def switch_to_deploy(self):
        for m in self.modules():
            if isinstance(m, (DWBlock_Std, DWBlock_ExpNoPRCM, DWBlock_Std_Bridge, DWBlock_ExpNoPRCM_Bridge)):
                m.switch_to_deploy()


if __name__ == "__main__":
    model = JeongWonNet_CtxBridge_StdExpNoPRCM_Nearest()
