"""
JeongWonNet_CtxBridge_StdExp_TConv + SCFFM Skip Fusion (Deterministic)

StdExp_TConv 기반에서 디코더 skip connection의 단순 덧셈(+)을
AMNet의 SCFFMModule (Spatial-Channel Feature Fusion Module)로 대체

기존:  d4 = up(decoder(d5)) + e4
변경:  d4 = scffm(e4, up(decoder(d5)))
       → encoder feat를 GAP으로 요약 + decoder feat에 SC attention 적용 후 채널 가중치 융합

얕은 레이어 (encoder1-4, decoder3-5): std(3x3)
깊은 레이어 (encoder5-6, decoder1-2): expand(1x1, x4)
업샘플링: ConvTranspose2d + BN
"""
import torch
...
class SCFFMModule(nn.Module):
    """Spatial-Channel Feature Fusion Module (from AMNet)

    x_low (encoder feat):  conv → GAP → 곱하기 channel_weight
    x_high (decoder feat): conv → SC attention → AdaptiveAvgPool → channel_weight
    output: x_low * channel_weight + x_high
    """
    def __init__(self, in_channels_low, in_channels_high, out_channels, reduction=4):
...
class JeongWonNet_CtxBridge_StdExp_TConv_SCFFM(nn.Module):
    """
    Context Bridge with Std Conv (shallow) + Expansion (deep)
    Upsampling: ConvTranspose2d + BN (deterministic)
    Skip Fusion: SCFFMModule (Spatial-Channel Feature Fusion, from AMNet)

    기존 단순 덧셈 skip connection을 SCFFM으로 대체:
        encoder feat (low) + decoder feat (high) → SCFFM → fused feat
    """
    def __init__(self,
                 num_classes=1,
                 input_channels=3,
        ...
        # SCFFM for skip connection fusion (replacing simple addition)
        # x_low=encoder feat, x_high=upsampled decoder feat
        self.scffm5 = SCFFMModule(c_list[4], c_list[4], c_list[4], reduction=scffm_reduction)
        self.scffm4 = SCFFMModule(c_list[3], c_list[3], c_list[3], reduction=scffm_reduction)
        self.scffm3 = SCFFMModule(c_list[2], c_list[2], c_list[2], reduction=scffm_reduction)
        self.scffm2 = SCFFMModule(c_list[1], c_list[1], c_list[1], reduction=scffm_reduction)
        self.scffm1 = SCFFMModule(c_list[0], c_list[0], c_list[0], reduction=scffm_reduction)
    ...
    def forward(self, x):
        ...
        # Decoder with SCFFM skip fusion
        d5 = self.scffm5(e5, self.decoder1(e6, ctx_enc=ctx5))
        d4 = self.scffm4(e4, self.up2(self.decoder2(d5, ctx_enc=ctx4)))
        d3 = self.scffm3(e3, self.up3(self.decoder3(d4, ctx_enc=ctx3)))
        d2 = self.scffm2(e2, self.up4(self.decoder4(d3, ctx_enc=ctx2)))
        d1 = self.scffm1(e1, self.up5(self.decoder5(d2, ctx_enc=ctx1)))
...
if __name__ == "__main__":
    model = JeongWonNet_CtxBridge_StdExp_TConv_SCFFM()
    ...
    scffm_params = sum(p.numel() for p in model.parameters())
    print(f"Base params:  {base_params:,}")
    print(f"SCFFM params: {scffm_params:,}")
    print(f"Overhead:     {scffm_params - base_params:,} (+{(scffm_params - base_params) / base_params * 100:.1f}%)")
