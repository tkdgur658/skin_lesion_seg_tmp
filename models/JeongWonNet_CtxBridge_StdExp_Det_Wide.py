"""
JeongWonNet_CtxBridge_StdExp_Det_Wide — 채널 확장 버전

원본 JeongWonNet_CtxBridge_StdExp_Det (~1M params)의 채널 폭을 확장하여
AMNet/MAResUNet 수준의 semantic capacity 확보.

채널 구성: [64, 128, 192, 256, 384, 512] → ~7.6M params
  - 원본:  [24,  48,  64,  96, 128, 192]
  - 확장:  [64, 128, 192, 256, 384, 512]

구조는 원본 Det과 완전 동일:
  - BilinearUp (고정 bilinear ConvTranspose2d) — 결정적 backward
  - 얕은 블록: DWBlock_Std (pw→dw 7x7→std 3x3→PRCM)
  - 깊은 블록: DWBlock_Exp (pw→dw 7x7→exp→proj→PRCM)
  - 디코더 Bridge: PRCM_Bridge
  - Deep supervision: nearest
  - RepConv 배포 모드 지원
"""
import torch
...
class BilinearUp(nn.Module):
    """Fixed (non-learnable) bilinear 2x upsampling via depthwise ConvTranspose2d.

    Forward: bilinear interpolation과 동일한 결과
    Backward: Conv2d forward와 동일 → deterministic
    """
    ...
    def _init_bilinear(self):
        ...
        center = kernel_size / 2.0 - 0.5
        og = torch.arange(kernel_size).float()
        filt_1d = 1 - torch.abs(og - center) / factor
        filt_2d = filt_1d.unsqueeze(0) * filt_1d.unsqueeze(1)

        channels = self.up.in_channels
...
class JeongWonNet_CtxBridge_StdExp_Det_Wide(nn.Module):
    """
    Context Bridge with Std Conv (shallow) + Expansion (deep) — Deterministic, Wide

    채널 확장 버전: [64, 128, 192, 256, 384, 512] → ~7.6M params
    원본 대비 채널 폭 증가로 공간 디테일 캡처 및 semantic capacity 강화

    디코더/최종출력: 고정 bilinear ConvTranspose2d (결정적 backward)
    딥 수퍼비전: nearest (보조 loss, 결정적)
    ...
    def __init__(self,
        ...
        # Fixed bilinear 2x upsampling (deterministic)
        self.up2 = BilinearUp(c_list[3])
        self.up3 = BilinearUp(c_list[2])
        self.up4 = BilinearUp(c_list[1])
        self.up5 = BilinearUp(c_list[0])
        self.up_final = BilinearUp(num_classes)
    ...
    def forward(self, x):
        ...
        # Decoder — fixed bilinear ConvTranspose2d (deterministic)
        ...
        out = self.up_final(self.final(d1))
...
if __name__ == "__main__":
    model = JeongWonNet_CtxBridge_StdExp_Det_Wide()
    ...
    out1 = model(x)
    out2 = model(x)
    print(f"Train forward deterministic: {torch.equal(out1[1], out2[1])}")
    ...
    print(f"Frozen (bilinear weights): {frozen:,}")
