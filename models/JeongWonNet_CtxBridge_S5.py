"""
JeongWonNet_CtxBridge with 5 Stages (1 stage removed)

기존 CtxBridge에서 bottleneck stage (encoder6) 제거
- Encoder: 5 stages (기존 6)
- Decoder: 4 stages (기존 5)
- 더 가벼운 모델, 더 낮은 receptive field
...
class JeongWonNet_CtxBridge_S5(nn.Module):
    """
    5-Stage Context Bridge (1 stage removed)

    기존 6 stage에서 bottleneck 제거
    - Encoder: 5 stages
    - Decoder: 4 stages
    - 더 가벼운 모델
    """
    def __init__(self,
        ...
        self.encoder5 = DWBlock(c_list[3], c_list[4], kernel_size, num_basis, dropout_rate)  # Bottleneck

        # Deep Supervision (5 outputs required)
        if gt_ds:
            self.gt_conv1 = nn.Conv2d(c_list[3], num_classes, 1)
            self.gt_conv2 = nn.Conv2d(c_list[2], num_classes, 1)
            self.gt_conv3 = nn.Conv2d(c_list[1], num_classes, 1)
            self.gt_conv4 = nn.Conv2d(c_list[0], num_classes, 1)
            self.gt_conv5 = nn.Conv2d(c_list[0], num_classes, 1)  # d1에서 두 번 사용

        # Decoder (4 stages)
        self.decoder1 = DWBlock_Bridge(c_list[4], c_list[3], c_list[3], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder2 = DWBlock_Bridge(c_list[3], c_list[2], c_list[2], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder3 = DWBlock_Bridge(c_list[2], c_list[1], c_list[1], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder4 = DWBlock_Bridge(c_list[1], c_list[0], c_list[0], kernel_size, num_basis, dropout_rate, bridge_dropout)
    ...
    def forward(self, x):
        ...
        # Bottleneck (no pooling after)
        e5 = self.encoder5(e4)

        # Decoder with context bridge
        d4 = self.decoder1(e5, ctx_enc=ctx4) + e4
        d3 = F.interpolate(self.decoder2(d4, ctx_enc=ctx3), scale_factor=2, mode='bilinear', align_corners=True) + e3
        d2 = F.interpolate(self.decoder3(d3, ctx_enc=ctx2), scale_factor=2, mode='bilinear', align_corners=True) + e2
        d1 = F.interpolate(self.decoder4(d2, ctx_enc=ctx1), scale_factor=2, mode='bilinear', align_corners=True) + e1
        ...
        if self.gt_ds and not is_eval:
            ...
            return (
                F.interpolate(self.gt_conv1(d4), (h, w), mode='bilinear', align_corners=True),
                F.interpolate(self.gt_conv2(d3), (h, w), mode='bilinear', align_corners=True),
                F.interpolate(self.gt_conv3(d2), (h, w), mode='bilinear', align_corners=True),
                F.interpolate(self.gt_conv4(d1), (h, w), mode='bilinear', align_corners=True),
...
if __name__ == "__main__":
    model = JeongWonNet_CtxBridge_S5()
    x = torch.randn(2, 3, 256, 256)

    model.train()
    ds_outputs, final_out = model(x)
    print(f"Training - DS outputs: {len(ds_outputs)}, Final: {final_out.shape}")
