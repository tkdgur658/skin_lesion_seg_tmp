"""
JeongWonNet with Context Bridge + DiffAware Fusion

Skip Connection 방식:
    High-level과 Low-level의 차이(diff)를 이용해 경계 attention 생성
    diff = |dec_feat - enc_feat| -> boundary_attn
    fused = dec_feat + enc_feat * boundary_attn

특징:
    - 경계선 정보를 명시적으로 포착
    - Semantic gap이 큰 영역을 강조
"""
import torch
...
class DiffAwareFusion(nn.Module):
    ...
    High-level과 Low-level의 차이를 이용해 경계 attention 생성
    """
...
class JeongWonNet_CtxBridge_DiffFusion(nn.Module):
    """
    Context Bridge + DiffAware Fusion Skip Connection

    경계 정보를 명시적으로 활용하여 high-level/low-level 융합
    """
    def __init__(self,
                 num_classes=1,
                 input_channels=3,
        ...
        # DiffAware Fusion modules
        self.fusion5 = DiffAwareFusion(c_list[4])
        self.fusion4 = DiffAwareFusion(c_list[3])
        self.fusion3 = DiffAwareFusion(c_list[2])
        self.fusion2 = DiffAwareFusion(c_list[1])
        self.fusion1 = DiffAwareFusion(c_list[0])
    ...
    def forward(self, x):
        ...
        # Decoder with context bridge + DiffAware fusion
...
if __name__ == "__main__":
    model = JeongWonNet_CtxBridge_DiffFusion()
