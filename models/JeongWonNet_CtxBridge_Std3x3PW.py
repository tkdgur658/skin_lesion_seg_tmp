"""
JeongWonNet_CtxBridge - Variant 1: 3x3 Standard Conv for Channel Adjustment

기존: pw_conv(1x1) → dw_conv(7x7) → prcm
변경: std_conv(3x3) → dw_conv(7x7) → prcm

채널 조정 시 공간 정보도 함께 활용
"""
import torch
...
class DWBlock(nn.Module):
    """DWBlock with 3x3 standard conv for channel adjustment"""
    def __init__(self, in_channels, out_channels, kernel_size=7, num_basis=8, dropout_rate=0.5):
        super().__init__()

        # 3x3 standard conv for channel adjustment (instead of 1x1)
...
class JeongWonNet_CtxBridge_Std3x3PW(nn.Module):
    """
    Context Bridge with 3x3 Standard Conv for Channel Projection
...
if __name__ == "__main__":
    model = JeongWonNet_CtxBridge_Std3x3PW()
