"""
JeongWonNet_CtxBridge_PostStdExp_mod + GlobalCtx

_mod 구조 (Std/Exp 블록 분리) + GlobalCtx 방식 (encoder1~5 GAP concat → decoder 전달)

얕은 레이어 (encoder1-4, decoder3-5):
    pw(1x1) → dw(7x7) → std(3x3) → prcm

깊은 레이어 (encoder5-6, decoder1-2):
    pw(1x1) → dw(7x7) → expand(1x1, x4) → project(1x1) → prcm

Decoder PRCM: global_ctx = cat(ctx1~ctx5) 사용
"""
import torch
...
class DWBlock_Std(nn.Module):
    ...
    def forward(self, x, return_ctx=False):
        ...
        if return_ctx:
            ...
            x = self.std_conv(x)
            return x, ctx
        return self.std_conv(self.prcm(x))
...
class DWBlock_Std_Bridge_Global(nn.Module):
    """Decoder DWBlock with 3x3 Standard Conv + GlobalCtx (for shallow layers)"""
    def __init__(self, in_channels, out_channels, enc_ctx_dim, kernel_size=7, num_basis=8, dropout_rate=0.5, bridge_dropout=0.2):
        super().__init__()
        ...
        self.prcm = PRCM_Bridge_Global(out_channels, enc_ctx_dim, num_basis=num_basis, dropout_rate=dropout_rate, bridge_dropout=bridge_dropout)
...
class DWBlock_Exp_Bridge_Global(nn.Module):
    """Decoder DWBlock with 1x1 Expansion-Projection + GlobalCtx (for deep layers)"""
    def __init__(self, in_channels, out_channels, enc_ctx_dim, kernel_size=7, num_basis=8, dropout_rate=0.5, bridge_dropout=0.2, expansion=4):
        super().__init__()
...
class JeongWonNet_CtxBridge_PostStdExp_mod_GlobalCtx(nn.Module):
    """
    PostStdExp_mod + GlobalCtx

    Encoder: Shallow(1-4) Std, Deep(5-6) Exp — 동일
    Decoder: per-stage ctx 대신 global_ctx = cat(ctx1~ctx5) 사용
    """
    def __init__(self,
                 num_classes=1,
                 input_channels=3,
        ...
        # Decoder - Deep layers with Expansion + GlobalCtx
        self.decoder1 = DWBlock_Exp_Bridge_Global(c_list[5], c_list[4], self.total_ctx_dim, kernel_size, num_basis, dropout_rate, bridge_dropout, expansion)
        self.decoder2 = DWBlock_Exp_Bridge_Global(c_list[4], c_list[3], self.total_ctx_dim, kernel_size, num_basis, dropout_rate, bridge_dropout, expansion)
        # Shallow layers with Std Conv + GlobalCtx
        self.decoder3 = DWBlock_Std_Bridge_Global(c_list[3], c_list[2], self.total_ctx_dim, kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder4 = DWBlock_Std_Bridge_Global(c_list[2], c_list[1], self.total_ctx_dim, kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder5 = DWBlock_Std_Bridge_Global(c_list[1], c_list[0], self.total_ctx_dim, kernel_size, num_basis, dropout_rate, bridge_dropout)
    ...
    def forward(self, x):
        ...
        d4 = F.interpolate(self.decoder2(d5, global_ctx=global_ctx), scale_factor=2, mode='bilinear', align_corners=True) + e4
        d3 = F.interpolate(self.decoder3(d4, global_ctx=global_ctx), scale_factor=2, mode='bilinear', align_corners=True) + e3
        d2 = F.interpolate(self.decoder4(d3, global_ctx=global_ctx), scale_factor=2, mode='bilinear', align_corners=True) + e2
        d1 = F.interpolate(self.decoder5(d2, global_ctx=global_ctx), scale_factor=2, mode='bilinear', align_corners=True) + e1
    ...
    def switch_to_deploy(self):
        for m in self.modules():
            if isinstance(m, (DWBlock_Std, DWBlock_Exp, DWBlock_Std_Bridge_Global, DWBlock_Exp_Bridge_Global)):
                m.switch_to_deploy()


if __name__ == "__main__":
    model = JeongWonNet_CtxBridge_PostStdExp_mod_GlobalCtx()
    x = torch.randn(2, 3, 256, 256)

    # Train mode test
    model.train()
    ds_out, out = model(x)
    print(f"Train - Output: {out.shape}, DS outputs: {len(ds_out)}")
