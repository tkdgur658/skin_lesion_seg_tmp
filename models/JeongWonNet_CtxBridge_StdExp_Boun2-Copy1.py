...
class PRCM(nn.Module):
    def __init__(self, channels, num_basis=16, dropout_rate=0.5):
        super().__init__()
        self.num_basis = num_basis
        self.channels = channels

        # basis: 2C input (GAP + GMP concat)
        self.basis = nn.Parameter(torch.randn(num_basis, 2 * channels))
        self.mlp = nn.Sequential(
            ...
            nn.LayerNorm(num_basis),
            nn.ReLU(inplace=True),
            nn.Linear(num_basis, channels, bias=False),
        )
    ...
    def forward(self, x, return_ctx=False):
        ...
        # Channel attention
        ctx_avg = x.mean(dim=[2, 3])
        ctx_max = F.adaptive_max_pool2d(x, 1).flatten(1)
        ctx = torch.cat([ctx_avg, ctx_max], dim=1)  # [B, 2C]
        coeff = ctx @ self.basis.t()                 # [B, 16]
        coeff = self.coeff_dropout(coeff)
        w_ch = self.mlp(coeff).sigmoid().unsqueeze(-1).unsqueeze(-1)
        x = x * w_ch

        # Spatial attention
        s = F.adaptive_avg_pool2d(x, 8)              # [B,C,8,8]
        s = self.spatial(s).sigmoid()                 # [B,C,8,8]
...
class DWBlock_Std(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=7, num_basis=16, dropout_rate=0.5):
        super().__init__()
