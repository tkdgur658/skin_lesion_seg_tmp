...
# QuantizationFriendlyRepConv
class RepConv(nn.Module):
    """
    Quantization-Friendly Re-parameterizable Convolution Block
    훈련: Conv + BN + Identity(or 1x1) branch + channel importance 추적
    추론: 단일 Conv로 융합
    """
    def __init__(self, in_channels, out_channels, kernel_size=3,
        ...
        # AWQ를 위한 channel importance 추적
        # ✅ 수정: out_ch → out_channels
        self.register_buffer('channel_importance', 
    ...
    def forward(self, x):
        ...
        # ✅ 수정: else → else:
            ...
            # ✅ 수정: activation 전에 importance 추적
            # Training 시에만 channel importance 업데이트
            if self.training:
                with torch.no_grad():
                    act_mag = out.abs().mean(dim=[0, 2, 3])
                    self.channel_importance = 0.9 * self.channel_importance + \
                                             0.1 * act_mag
...
class JeongWonNet77_Rep256Basis8S24_QuanRep(nn.Module):
    """
