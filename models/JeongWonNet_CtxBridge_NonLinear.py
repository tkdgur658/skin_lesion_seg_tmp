"""
JeongWonNet_CtxBridge Variant: NonLinear Projection

변경점:
    기존: ctx_proj = Linear(ctx_enc)  (단순 선형 변환)
    변경: ctx_proj = GELU(LayerNorm(Linear(ctx_enc)))  (비선형 변환)
...
class PRCM_Bridge_NonLinear(nn.Module):
    """
    PRCM with NonLinear Projection

    기존: ctx_proj = Linear(ctx_enc)
    변경: ctx_proj = Dropout(GELU(LayerNorm(Linear(ctx_enc))))

    구조:
        ctx_enc → Linear → LayerNorm → GELU → Dropout → ctx_enc_proj
    ...
    def __init__(self, channels, enc_channels, num_basis=8, dropout_rate=0.5, bridge_dropout=0.2):
        ...
        # NonLinear context projection with dropout
        self.ctx_proj = nn.Sequential(
            nn.Linear(enc_channels, channels, bias=False),
            nn.LayerNorm(channels),
            nn.GELU(),
            nn.Dropout(bridge_dropout) if bridge_dropout > 0 else nn.Identity()
        )
...
class DWBlock_Bridge_NonLinear(nn.Module):
    def __init__(self, in_channels, out_channels, enc_channels, kernel_size=7, num_basis=8, dropout_rate=0.5, bridge_dropout=0.2):
        ...
        self.prcm = PRCM_Bridge_NonLinear(out_channels, enc_channels, num_basis=num_basis, dropout_rate=dropout_rate, bridge_dropout=bridge_dropout)
...
class JeongWonNet_CtxBridge_NonLinear(nn.Module):
    """
    CtxBridge with NonLinear Projection

    ctx_enc → Linear → LayerNorm → GELU → ctx_enc_proj
    ctx_fused = ctx_self + ctx_enc_proj
    """
    def __init__(self,
                 num_classes=1,
        ...
        # Decoder with NonLinear Bridge (bridge_dropout 적용)
        self.decoder1 = DWBlock_Bridge_NonLinear(c_list[5], c_list[4], c_list[4], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder2 = DWBlock_Bridge_NonLinear(c_list[4], c_list[3], c_list[3], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder3 = DWBlock_Bridge_NonLinear(c_list[3], c_list[2], c_list[2], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder4 = DWBlock_Bridge_NonLinear(c_list[2], c_list[1], c_list[1], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder5 = DWBlock_Bridge_NonLinear(c_list[1], c_list[0], c_list[0], kernel_size, num_basis, dropout_rate, bridge_dropout)
    ...
    def switch_to_deploy(self):
        for m in self.modules():
            if isinstance(m, (DWBlock, DWBlock_Bridge_NonLinear)):
                m.switch_to_deploy()


if __name__ == "__main__":
    print("=" * 60)
    print("JeongWonNet_CtxBridge_NonLinear Test")
    print("Projection: Linear → LayerNorm → GELU")
    print("=" * 60)

    model = JeongWonNet_CtxBridge_NonLinear(
        num_classes=1,
        input_channels=3,
        c_list=[24, 48, 64, 96, 128, 192],
        kernel_size=7,
    ...
    print(f"NonLinear Projection Parameters: {proj_params:,}")

    print("\n[NonLinear Projection 구조]")
    print("  ctx_enc → Linear → LayerNorm → GELU → ctx_enc_proj")
    print("  ctx_fused = ctx_self + ctx_enc_proj")
    print("  복잡한 채널 상호작용 학습, 안정적 학습")
