"""
JeongWonNet_CtxBridge_Res

DWBlock에 Residual Connection 추가

구조:
    x ─────────────────────────────────┐
    │                                  │ (identity or 1x1 conv)
    ↓                                  ↓
    [1x1 Conv] → RepConv 7x7 DW → PRCM → + → output
...
class DWBlockRes(nn.Module):
    """
    DWBlock with Residual Connection (Encoder)

    구조:
        x ─────────────────────────────────┐
        │                                  │ shortcut (identity or 1x1+BN)
        ↓                                  ↓
        [1x1 Conv] → RepConv 7x7 DW → PRCM → + → output
    """
    def __init__(self, in_channels, out_channels, kernel_size=7, num_basis=8, dropout_rate=0.5):
        ...
        # Shortcut connection with BN for scale matching
    ...
    def forward(self, x, return_ctx=False):
        ...
        if return_ctx:
            out, ctx = self.prcm(out, return_ctx=True)
            return out + identity, ctx
...
class DWBlockRes_Bridge(nn.Module):
    """
    DWBlock with Residual Connection + Context Bridge (Decoder)

    구조:
        x ─────────────────────────────────────────┐
        │                                          │ shortcut (identity or 1x1+BN)
        ↓                                          ↓
        [1x1 Conv] → RepConv 7x7 DW → PRCM_Bridge → + → output
                                        ↑
                                     ctx_enc
    """
...
class JeongWonNet_CtxBridge_Res(nn.Module):
    """
    Context Bridge with Residual DWBlock

    DWBlock에 residual connection 추가:
        output = DWBlock(x) + shortcut(x)
    ...
    def __init__(self,
        ...
        self.encoder1 = DWBlockRes(input_channels, c_list[0], kernel_size, num_basis, dropout_rate)
        self.encoder2 = DWBlockRes(c_list[0], c_list[1], kernel_size, num_basis, dropout_rate)
        self.encoder3 = DWBlockRes(c_list[1], c_list[2], kernel_size, num_basis, dropout_rate)
        self.encoder4 = DWBlockRes(c_list[2], c_list[3], kernel_size, num_basis, dropout_rate)
        self.encoder5 = DWBlockRes(c_list[3], c_list[4], kernel_size, num_basis, dropout_rate)
        self.encoder6 = DWBlockRes(c_list[4], c_list[5], kernel_size, num_basis, dropout_rate)
        ...
        # Decoder with Residual + Bridge
        self.decoder1 = DWBlockRes_Bridge(c_list[5], c_list[4], c_list[4], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder2 = DWBlockRes_Bridge(c_list[4], c_list[3], c_list[3], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder3 = DWBlockRes_Bridge(c_list[3], c_list[2], c_list[2], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder4 = DWBlockRes_Bridge(c_list[2], c_list[1], c_list[1], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder5 = DWBlockRes_Bridge(c_list[1], c_list[0], c_list[0], kernel_size, num_basis, dropout_rate, bridge_dropout)
    ...
    def switch_to_deploy(self):
        for m in self.modules():
            if isinstance(m, (DWBlockRes, DWBlockRes_Bridge)):
                m.switch_to_deploy()


if __name__ == "__main__":
    print("=" * 60)
    print("JeongWonNet_CtxBridge_Res Test")
    print("DWBlock with Residual Connection")
    print("=" * 60)

    model = JeongWonNet_CtxBridge_Res(
        num_classes=1,
        input_channels=3,
        c_list=[24, 48, 64, 96, 128, 192],
        kernel_size=7,
    ...
    # Shortcut params (채널이 다른 경우만)
    shortcut_params = 0
    for m in model.modules():
        if isinstance(m, (DWBlockRes, DWBlockRes_Bridge)):
            if isinstance(m.shortcut, nn.Conv2d):
                shortcut_params += sum(p.numel() for p in m.shortcut.parameters())
    print(f"Shortcut Parameters: {shortcut_params:,}")

    print("\n[Residual DWBlock 구조]")
    print("  x ───────────────────────────┐")
    print("  │                            │ shortcut")
    print("  ↓                            ↓")
    print("  [1x1] → RepConv DW → PRCM → (+) → output")
