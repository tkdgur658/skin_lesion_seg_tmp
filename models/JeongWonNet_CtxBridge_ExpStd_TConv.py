"""
JeongWonNet_CtxBridge_ExpStd with Transposed Convolution Upsampling (Deterministic)

ExpStd의 업샘플링을 F.interpolate(bilinear) 대신 ConvTranspose2d로 대체한 버전
- 디코더 업샘플링: ConvTranspose2d + BN (learnable, deterministic)
- final/deep supervision 업샘플링: nearest interpolation (deterministic)
- bilinear interpolation 완전 제거 → CUDA backward 재현성 보장
...
def _init_weights(m):
    if isinstance(m, nn.Conv2d):
        ...
        # ConvTranspose2d weight shape: (in_ch, out_ch, kH, kW) — use fan_in
...
class JeongWonNet_CtxBridge_ExpStd_TConv(nn.Module):
    """
    Context Bridge with Expansion (shallow) + Std Conv (deep)
    Upsampling: ConvTranspose2d (instead of bilinear interpolation)
    ...
    def __init__(self,
        ...
        # Encoder - Shallow layers with Expansion
        ...
        # Decoder - Deep layers with Std Conv
        self.decoder1 = DWBlock_Std_Bridge(c_list[5], c_list[4], c_list[4], kernel_size, num_basis, dropout_rate, bridge_dropout)
        self.decoder2 = DWBlock_Std_Bridge(c_list[4], c_list[3], c_list[3], kernel_size, num_basis, dropout_rate, bridge_dropout)
        # Shallow layers with Expansion
        self.decoder3 = DWBlock_Exp_Bridge(c_list[3], c_list[2], c_list[2], kernel_size, num_basis, dropout_rate, bridge_dropout, expansion)
        self.decoder4 = DWBlock_Exp_Bridge(c_list[2], c_list[1], c_list[1], kernel_size, num_basis, dropout_rate, bridge_dropout, expansion)
        self.decoder5 = DWBlock_Exp_Bridge(c_list[1], c_list[0], c_list[0], kernel_size, num_basis, dropout_rate, bridge_dropout, expansion)

        # Transposed Convolution for upsampling (replacing F.interpolate)
        # BN after ConvTranspose2d to stabilize scale before skip connection addition
        ...
        # final upsampling: keep bilinear (only num_classes=1 channel, too few params for TConv)
...
if __name__ == "__main__":
    model = JeongWonNet_CtxBridge_ExpStd_TConv()
